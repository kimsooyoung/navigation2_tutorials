...
void SMSRecovery::onConfigure()
{
  ...
  auto node = std::make_shared<rclcpp_lifecycle::LifecycleNode>("sms_recovery_on_config");
...
{
  // auto node = node_.lock();
  auto node = std::make_shared<rclcpp_lifecycle::LifecycleNode>("sms_recovery_on_run");
  std::string response;
